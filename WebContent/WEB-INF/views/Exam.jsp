			...
			<div class="row">
				...
				<div class="col-sm-9 padding-right">
					<div class="features_items">
						...
						<table >
							<tr>
								<td><button type="button" class="btn  btn-warning ">Part1</button></td>
								<td><button type="button" class="btn  btn-warning ">Part2</button></td>
								<td><button type="button" class="btn  btn-warning ">Part3</button></td>
							</tr>
						</table>
						</div><br>
						...
						<c:if test="${not empty Question.imagequestion }">
								<div><img src="images/examinationquestion/${Question.imagequestion }" width="240px" height="160px"></img></div>	
						</c:if>
						<c:if test="${not empty Question.audiomp3 }">
							<div> <audio controls> <source src="Audio/examinationquestion/${Question.audiomp3 }" type="audio/mpeg"></audio></div>
						</c:if>
						<c:if test="${not empty Question.option1}">${Question.option1}<br></c:if>
						<c:if test="${not empty Question.option2}">${Question.option2}<br></c:if>
						<c:if test="${not empty Question.option3}">${Question.option3}<br></c:if>
						<c:if test="${not empty Question.option4}">${Question.option4}<br></c:if>
						<br>
						 <c:if test="${stt>1}">
            	...
            	<form  action="" method="post" id="check_form">	
            		...
            		<tr>
            		<td><a href="javascript:clickSTT(<%=i%>)" class="hrefQuestion" value="<%=i%>"><%=i+1%>&nbsp&nbsp</a></td>
            	...
            	<button id= "NopBai" type="button" class="btn  btn-warning ">Nộp  bài</button>
				</div>
				</div>
