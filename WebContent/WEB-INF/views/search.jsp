			...
			<div class="row">
				...
				<div class="col-sm-6 padding-right">
					<div class="features_items">
						<!--features_items-->
						<h2 class="title text-center">Kết quả tìm kiếm cho từ khóa : '${key}'</h2>
							...
							<div class="container col-sm-12">	
								<div class="container">
								  <ul class="pagination">
								  	 	<c:if test="${Page!=1}" ><li class=><a  href="SearchAll?key=${key}&page=${Page-1}">Prev</a></li></c:if>
								  	 <c:forEach var="item" items="${ListPage}">
								  	 	<c:if test="${item==Page}" ><li class="active"><a  href="SearchAll?key=${key}&page=${item}">${item}</a></li></c:if>
								  		<c:if test="${item!=Page}" ><li><a href="SearchAll?key=${key}&page=${item}">${item}</a></li></c:if>
								  	 </c:forEach>
										<c:if test="${Page!=TotalPage}" ><li class=><a  href="SearchAll?key=${key}&page=${Page+1}">Next</a></li></c:if>
